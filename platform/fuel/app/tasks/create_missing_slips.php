...
class Create_Missing_Slips extends Task_Cli
{
    ...
    public function run(): void {
        $pending_tickets = DB::query("SELECT w.name, wut.date, wt.date_confirmed, wut.whitelabel_id, wut.draw_date, l.name
        AS lottery_name, l.timezone, wut.id AS ticket_id, wut.token,
        (SELECT COUNT(*)
        FROM whitelabel_user_ticket_slip wuts
        WHERE wuts.whitelabel_user_ticket_id = wut.id) AS count
