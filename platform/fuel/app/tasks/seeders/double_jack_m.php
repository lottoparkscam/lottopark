...
final class Double_Jack_M extends Seeder
{
    use \Without_Foreign_Key_Checks,
        \Enables_Lottery_On_Development,
        \Adjusts_Next_Draw_Date;

    const LOTTERY_ID = Helpers_Lottery::DOUBLE_JACK_M_ID;
    const LOTTERY_SOURCE_ID = 44;
    const LOTTERY_TYPE_ID = 38;
    const LOTTERY_PROVIDER_ID = 48;
    ...
    protected function rowsStaging(): array
    {
        ...
        return [
            'lottery' => [
                [self::LOTTERY_ID, self::LOTTERY_SOURCE_ID, 'DoubleJack Million', 'DJM', 'World', 'FR', 'double-jack-million', 'is_enabled' => 0, $timezone, $draw_dates_json, 0, Currency::USD, 0, 0, 0.00, Carbon::now()->format(Helpers_Time::DATE_FORMAT), 0.4, 0, $draw_dates['next_date_local'], $draw_dates['next_date_utc'], $draw_dates['last_date_local'], 1, 1],
            ],
            'lottery_source' => [
            ...
            'whitelabel_lottery' => [
                ['26', self::LOTTERY_ID, self::LOTTERY_PROVIDER_ID, '0', '0', '0.60', '0', '0', '1000', '5'],
            ],
        ];
    }
}
