...
final class Double_Jack extends Seeder
{
    use \Without_Foreign_Key_Checks,
        \Enables_Lottery_On_Development,
        \Adjusts_Next_Draw_Date;

    const LOTTERY_ID = Helpers_Lottery::DOUBLE_JACK_ID;
    const LOTTERY_SOURCE_ID = 42;
    const LOTTERY_TYPE_ID = 36;
    const LOTTERY_PROVIDER_ID = 46;
    ...
    protected function rowsStaging(): array
    {
        ...
        return [
            'lottery' => [
                [self::LOTTERY_ID, self::LOTTERY_SOURCE_ID, 'DoubleJack', 'DJ', 'World', 'FR', 'double-jack', 'is_enabled' => 0, $timezone, $draw_dates_json, 0, Currency::USD, 0, 0, 0.00, Carbon::now()->format(Helpers_Time::DATE_FORMAT), 3.6, 0, $draw_dates['next_date_local'], $draw_dates['next_date_utc'], $draw_dates['last_date_local'], 1, 1],
            ],
            'lottery_source' => [
            ...
            'whitelabel_lottery' => [
                ['26', self::LOTTERY_ID, self::LOTTERY_PROVIDER_ID, '0', '0', '1.00', '0', '0', '1000', '1'],
            ],
        ];
    }
}
