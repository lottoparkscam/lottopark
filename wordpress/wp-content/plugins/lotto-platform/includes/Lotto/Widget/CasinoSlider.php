...
class Lotto_Widget_CasinoSlider extends WP_Widget
{
    private int $whitelabelId;
    private ?Whitelabel $whitelabel;
    private const CACHE_KEY = 'casinoSliderWidget_';
    ...
    public function __construct()
    {
        ...
        $sliderId = 'lotto_platform_widget_slot_games_slider';

        parent::__construct(
            $sliderId,
            _('Lotto Casino Slider'),
            [
     ...
     * @var string $cacheKey example: casinoSliderWidget_pl_30
     */
    public function widget($args, $instance)
    {
        $shouldBlockSlider = IS_CASINO && is_front_page();
        if ($shouldBlockSlider) {
            return;
        }
        ...
        $casinoSliderJs = AssetHelper::mix('js/slots/Widget.min.js', AssetHelper::TYPE_WORDPRESS, true);
        ...
        $internalInitUrl = RouteHelper::getCasinoPlayLink($this->whitelabel->domain);
        $internalInitUrl = UrlHelper::changeAbsoluteUrlToCasinoUrl($internalInitUrl, true);
        $lobbyInitUrl = UrlHelper::changeAbsoluteUrlToCasinoUrl(
            RouteHelper::getCasinoLobbyLink($this->whitelabel->domain),
            true
        );

        $language = ICL_LANGUAGE_CODE;
        $wlCasinoExists = (int)!empty(RouteHelper::getCasinoPlayLink($this->whitelabel->domain));
        ...
        echo "<script>
                ...
                const CASINO_INTERNAL_INIT_URL = '{$internalInitUrl}';
                const CASINO_LANGUAGE = '{$language}';
                const CASINO_WIDGET_TITLE = '{$title}';
                const CASINO_IS_EXISTS = '{$wlCasinoExists}';
                const CASINO_LOBBY_INIT_URL = '{$lobbyInitUrl}';
