 ...
 * @deprecated
 */
final class AstroPaymentForm extends Forms_Wordpress_Payment_Base implements Forms_Wordpress_Payment_Process
{
    use Traits_Payment_Method;
    use Traits_Payment_Method_Currency;

    protected $payment_method = 32;
    protected string $methodSlug = 'astro';
    private $request = [];
    ...
    public function process_form(): void
    {
        ...
        $logger->logInfo('Starting Astro payment', [
            'transaction' => $transaction
        ]);

        /** @var AstroFacade $facade */
        ...
        try {
            $url = $facade->requestCheckoutUrl($token, $this->whitelabel['id'], $amount, $currency);
        } catch (Exception $e) {
    ...
    private function set_settings_by_whitelabel_payment_method_id(
        ...
        // which is given from webhook on Astro page
    ...
    public function confirm_payment(
        ...
        try {
            ...
            $t = Container::get(TransactionRepository::class)->getByToken($requestData['token'], Container::get('whitelabel')->id);
            ...
            if ($wasSuccessful && $t->status == Helpers_General::STATUS_TRANSACTION_PENDING) {
                try {
