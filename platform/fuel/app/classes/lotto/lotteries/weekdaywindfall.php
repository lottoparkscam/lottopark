...
class Lotto_Lotteries_WeekdayWindfall extends Lotto_Lotteries_Lottery
{
    use Lotto_Hasscraping;

    protected string $lottery_slug = 'weekday-windfall';
    ...
    public function getDataPrimary(): array
    {
        $scraper = Lotto_Scraperapi::build()->fetchJsonDataStructureWithParameters('https://data.api.thelott.com/sales/vmax/web/data/lotto/latestresults', [
            ...
            'OptionalProductFilter' => ['MondayWednesdayFridayLotto'],
        ]);
        ...
        $lastDrawDateFormatted = $lastDrawDateFormatted = $lastDrawDate->format('Ymd');

        if (
            $this->nextDrawDate->format('Ymd') !== $drawDateTimeFormatted &&
