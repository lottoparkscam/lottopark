...
/**
 * White-Lotto Part:
 * Remember, there is no FuelPHP yet. This is very early stage of Wordpress load
 * More information:
 * https://gginternational.slite.com/app/channels/4mtH3PN_5R/notes/uNsaeyzYPH
 */
...
global $casinoConfig;
try {
    $projectDir = $_SERVER['DOCUMENT_ROOT'] ?? '/var/www/wordpress';
    $urlHelperPath = $projectDir . '/../platform/fuel/app/classes/helpers/UrlHelper.php';
    if (is_file($urlHelperPath)) {
        require_once($urlHelperPath);
    }
    $casinoConfigPath = $projectDir . '/../platform/fuel/app/config/casino.php';
    if (is_file($casinoConfigPath)) {
        $casinoConfig = require_once($casinoConfigPath);
    } else {
        $casinoConfig = [
            'prefixesMap' => [],
            'slugMap' => [],
...
require_once('plugins/lotto-platform/includes/wordpress_in_fuel.php');
$wordpress_in_fuel = new WordpressInFuel();
$wordpress_in_fuel->runSunrise();
