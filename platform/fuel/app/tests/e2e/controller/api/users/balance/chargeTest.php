...
class Tests_E2e_Controller_Api_Users_Balance_Charge extends Test_E2e_Controller_Api
{
    ...
    public function setUp(): void
    {
        ...
        $this->method = "PATCH";
        $this->endpoint = "/api/users/balance/charge";
    ...
    /** @test  */
    public function add_balance_via_email()
    {
        // amount send via api => should it work
        $set_of_critical_amounts_block_by_daily_limit = [
            -5 => false,
            0 => false,
            1 => true,
            99 => true,
            2 => false, // because is bigger than daily limit
        ];

       $this->change_balance_main_cases($set_of_critical_amounts_block_by_daily_limit, $this->email);

        $this->whitelabel->set([
            'max_daily_balance_change_per_user' => 301,
            'user_balance_change_limit' => 200
        ]);
        $this->whitelabel->save();
        $this->reset_balance_history();
        ...
        // amount send via api => should it work
        $set_of_critical_amounts_block_by_global_limit = [
            -5 => false,
            0 => false,
            1 => true,
            199 => true,
            2 => false, // because is bigger than global limit
        ];

        $this->change_balance_main_cases($set_of_critical_amounts_block_by_global_limit, $this->email);


        // check other scenario
        // where is_balance_change_global_limit_enabled_in_api is false
        $this->whitelabel->set([
            'max_daily_balance_change_per_user' => 900,
            'user_balance_change_limit' => 200,
    ...
    /** @test  */
    public function add_balance_via_incorrect_email()
    {
    ...
    /** @test  */
    public function add_balance_via_email_if_user_is_deleted()
    {
        $email = 'test@user.loc';
    ...
    /** @test  */
    public function add_balance_via_login_if_email_login_only_enabled()
    {
        ...
        $expected_response = [
            ...
            'errors' =>
                [
                    'title' => 'Bad request',
                    'message' => ['user_email' => 'Field user_email is required'],
                ]
        ];
    ...
    /** @test  */
    public function add_balance_via_login()
    {
        // amount send via api => should it work
        $set_of_critical_amounts_block_by_daily_limit = [
            -5 => false,
        ...
        $this->change_balance_main_cases($set_of_critical_amounts_block_by_daily_limit, '', $login);
        ...
        $this->change_balance_main_cases($set_of_critical_amounts_block_by_global_limit, '', $login);
    }

    /** @test  */
    public function add_balance_via_incorrect_login()
    {
        $login = 'tojestlogindotestow';
    ...
    /** @test  */
    public function add_balance_via_login_if_user_is_deleted()
    {
        $login = 'tojestlogindotestow';
        $this->set_logins_for_users_for_whitelabel('tojestlogindotestow');
    ...
    /** @test  */
    public function add_balance_via_email_if_login_by_login_only_enabled()
    {
    ...
    /** @test  */
    public function add_balance_via_email_if_currency_code_not_exists()
    {
        $email = 'test@user.loc';
    ...
    /** @test  */
    public function add_balance_via_email_if_currency_code_is_other_than_users()
    {
        $email = 'test@user.loc';
        $currencyCode = 'PLN';
        ...
        $previousUserBalanceChangeLimit = $this->whitelabel->user_balance_change_limit;
        $amount = 100;
        ...
        $newUserBalanceChangeLimit = $this->whitelabel->user_balance_change_limit;

        $this->assertSame($previousUserBalance + $amountInUsd, $newBalance);
        $this->assertSame($previousUserBalanceChangeLimit - $amountInWhitelabelCurrency, $newUserBalanceChangeLimit);
    ...
    /** @test  */
    public function add_balance_via_login_if_currency_code_not_exists()
    {
        $this->set_logins_for_users_for_whitelabel('samelogin');
     ...
     * Check blocking to increase balance depending on daily limits per user
     * Check blocking to increase balance depending on global bonus limit
        ...
        foreach ($data_set as $amount => $should_work) {
            ...
            if ($should_work) {
                ...
                if ($this->whitelabel->is_balance_change_global_limit_enabled_in_api) {
                    ...
                    // check if global limit has been reduced
                    $this->assertSame(
                ...
                // check if user's balance has been changed
                ...
                //check if user's balance has not been changed
                $this->assertSame($previous_user_balance, $this->user->balance);

                // check if global limit has not been reduced
    ...
    private function reset_balance_history(): void
    {
        ...
        if (!empty($change_balance_limit_logs)) {
            /** @var Model_Whitelabel_User_Balance_Change_Limit_Log $change_balance_limit_log */
