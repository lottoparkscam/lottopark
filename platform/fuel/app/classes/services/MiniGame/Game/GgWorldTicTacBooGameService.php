...
class GgWorldTicTacBooGameService extends AbstractMiniGameService implements MiniGameServiceInterface
{
    public const GAME_SLUG = MiniGame::GG_WORLD_TIC_TAC_BOO_SLUG;
    ...
    public function generateWinningGrid(int $winningSymbol): array
    {
        ...
        foreach ($combinations as $combo) {
            ...
            foreach ($combo as $position) {
                $row = $position[0];
                $col = $position[1];
                $testGrid[$row][$col] = $winningSymbol;
            }
            ...
            foreach ($remainingCells as $position) {
                $row = $position[0];
                $col = $position[1];
                $testGrid[$row][$col] = $losingSymbol;
            }

            $winningSymbolLines = $this->countWinningLines($testGrid, $winningSymbol, $winningLines);
            $losingSymbolLines = $this->countWinningLines($testGrid, $losingSymbol, $winningLines);
