...
abstract class AbstractWrapper
{
    ...
    {
        if (in_array($this->_rootTag, ['multipay', 'paycode', 'billcode'])) {
     ...
     * 
     */
    public function getErrorsString($paymentMethod = 'all', $message = [])
    {
    ...
    {
        $supportedErrorsPos = ['global', 'sr', 'su'];
        ...
        $error = ['code' => $errorCode, 'message' => $message, 'field' => $field];
    ...
    protected function _setNotification($notificationAddress, $notificationType, $notifyOn = '')
    {
        if ($notifyOn) {
            ...
            $notifyOnDefault = ['loss', 'pending', 'received', 'refunded'];
            $notifyOnArray = [];
            ...
            $notifyOn = ['notify_on' => implode(',', $notifyOnArray)];
        }
        
        if (!$notifyOn) {
            $notification = ['@data' => $notificationAddress];
        } else {
            $notification = ['@data' => $notificationAddress, '@attributes' => $notifyOn];
        }
