...
/** Bootstrap for all wordpress routes */
// FUEL 1.8


if (!defined('WORDPRESS_INSIDE_FUEL')) {
    $dir = realpath(ABSPATH.'../platform/public');
...
$maintenanceService = new Services\MaintenanceService();
$maintenanceService->setMaintenanceDomainFilename(APPPATH . '.maintenance-domain');
if ($maintenanceService->isDomainUnderMaintenance()) {
 ...
 * Fuel::PRODUCTION
 */
if (file_exists(APPPATH."../../../wordpress/.env")) {
    $_ENV['FUEL_ENV'] = file_get_contents(APPPATH."../../../wordpress/.env");
}
...
function fuel_set_wordpress_die_handler($original)
{
    return 'fuel_wordpress_die_handler';
}

function fuel_wordpress_die_handler($message, $title = '', $args = array())
{
    // fuel will take over
}

function fuel_shutdown()
{
...
// overwrite wordpress default wp die handler
add_filter('wp_die_handler', 'fuel_set_wordpress_die_handler');

// call fuel shutdown before wordpress shutdown
add_action('shutdown', 'fuel_shutdown', 0);
...
function get_composer()
{
    ...
    // NOTICE! This variable is shown as uninitialized by my IDE!
    static $composer;
