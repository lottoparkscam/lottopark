...
class Lotto_Lotteries_LaPrimitiva extends Lotto_Lotteries_Lottery
{
    use Lotto_Hasscraping;

    protected string $lottery_slug = Lottery::LA_PRIMITIVA_SLUG;
    ...
    public function get_results(): void
    {
        ...
        try {
            ...
            $scrapper = Lotto_Scraperhtml::build('https://www.elgordo.com/en/results/primitiva', ScraperHelper::$NUMBER_LOCALE_COMMA_DOT)
                ->setInitialBoundaries('<div class="main">', '<div class="title" role="game-title">Past results</div>')
                ->setDrawDateBoundaries('<div class="draw-result">', '</span>')
                ->setNumbersBoundaries('<span class="bet-break"><span class="numbers">', '<span class="text">R</span>')
                ->setPrizesBoundaries('<th class="d last-visible">Prize</th>', '</table>');
            ...
            if (empty($jackpot) || empty($numbers[0]) || empty($numbers[1]) || empty($prizes) || empty($drawDateTime)) {
                throw new Exception('[First source] Cannot retrieve data for the La Primitiva lottery or the retrieved data is empty.');
            }

            try {
                $this->validateResults([$jackpot, $drawDate, $numbers[0], [$numbers[1][0]], $prizes, ['refund' => (int)$numbers[1][1]]], 6, 1, [1, 49], [1, 49], 7, ['refund', 0, 9]);
    ...
    public function match_others($match_others, $line): int
    {
        ...
        if (
            $refund === $this->additional_data['refund'] &&
