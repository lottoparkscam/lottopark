...
class Forms_Whitelabel_Bonuses_Promocodes_Edit extends Forms_Main
{
    /**
     *
     * @var array
    ...
    */
    ...
    public static function _validation_unique($val, $token)
    {
    ...
    {
        $inside = View::forge("whitelabel/bonuses/promocodes/edit.php");

        $campaign = Model_Whitelabel_Campaign::find_by(['token' => $this->token]);
        $this->campaign = $campaign[0];

        list($codes_count, $clength) = Model_Whitelabel_Promo_Code::get_promo_codes_count_for_campaign($this->campaign['id']);
        ...
        $this->campaign_used = Model_Whitelabel_Campaign::is_used($this->token);
                    
        $inside->set('currency_code', $currency->code);
        $inside->set('lotteries', $this->lotteries);
        $inside->set('campaign', $this->campaign);
        $inside->set('codes_count', (int)$codes_count);
        $inside->set('code_length', $clength);
        $inside->set('used', $this->campaign_used);

        $affId = $this->campaign['whitelabel_aff_id'];
        if (!empty($affId)) {
            $aff = $this->whitelabelAffRepository->findAffiliateById($affId);
            $inside->set('affiliateEmail', $aff['email']);
        } else {
            $inside->set('affiliateEmail', '');
        }
        ...
        if (Input::post()) {
            ...
            if ($val->run()) {
                if ($this->campaign_used) {
                    ...
                    $this->campaign->set($campaign_values);
                    $this->campaign->save();

                    Session::set_flash("message", ["success", _("Campaign has been saved!")]);
                } else {
                ...
                "m/d/Y"
                    ...
                    if ((int)$val->validated("input.codes_type") === 1) {
                        ...
                        if (($codes_num <> $codes_count) || ($code_length <> $clength)) {
                            Model_Whitelabel_Promo_Code::delete_promo_codes_for_campaign($id);
                            ...
                            for ($i = 0; $i < $codes_num; $i++) {
                                ...
                                array_push($codes_data, [$token, $id]);
                            }
