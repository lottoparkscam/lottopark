...
$skrillemail_error_class = '';
if (isset($this->errors['withdrawal.add.skrill_email'])) {
    $skrillemail_error_class = ' has-error';
}
$user_s = '';
if (Input::post("withdrawal.add.skrill_email") !== null) {
    $user_s = stripslashes(Input::post("withdrawal.add.skrill_email"));
}
$user_skrillemail = htmlspecialchars($user_s);
...
<div class="form-group <?= $skrillemail_error_class; ?>">
    <label for="inputSkrillEmail">
        <?= Security::htmlentities(_("Skrill E-mail")); ?>:
    </label>
    <input type="email" required 
           value="<?= $user_skrillemail; ?>" 
           class="form-control" 
           id="inputSkrillEmail" 
           name="withdrawal[add][skrill_email]"
           placeholder="<?= htmlspecialchars(_("Your Skrill e-mail")); ?>">
</div>
