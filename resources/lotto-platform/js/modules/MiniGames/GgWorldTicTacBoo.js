...
const MiniGame = (() => {
  ...
  const updateGameHistoryDisplay = (history) => {
    ...
    history.forEach((item) => {
      ...
      gameHistoryItem.innerHTML = `
        <img src="${item.user_selected_number == 0 ? window.pumpkinImg : window.ghostImg}" alt="${item.user_selected_number == 0 ? 'Pumpkin' : 'Ghost'}" />
  ...
  const initGameGrid = (jsonGrid, handleTileClick) => {
    ...
    jsonGrid.forEach((row, rowIndex) => {
      row.forEach((cell, cellIndex) => {
        ...
        const icon = cell === 0 ? window.pumpkinImg : window.ghostImg;
        const alt = cell === 0 ? 'Pumpkin Icon' : 'Ghost Icon';
  ...
  const updateGameHistory = (isWin, userSelectedNumber, prize, amount, isFreeSpin, isUsedBonusBalance) => {
    ...
    gameHistoryItem.innerHTML = `
      <img src="${userSelectedNumber === 0 ? pumpkinImg : ghostImg}" alt="${userSelectedNumber === 0 ? 'Pumpkin' : 'Ghost'}" />
      <strong>${isWin ? 'Win' : 'Loss'}</strong>
  ...
  const showWinningLine = (winningLine, revealedItems) => {
    ...
    const lineColorClass = winningLine.symbol === 0 ? 'orange' : 'white';
  ...
  const showPromoCodeMessage = (message, scrollToMessage) => {
    ...
    let messageHTML = `
      <div class="gameBoxMessage">
          <img src="${window.happyGhostImg}" alt="Ghost" />
    ...
    playButton.addEventListener('click', (e) => {
      if (!playButton.href.includes(depositUrl)) {
        ...
        isGameInProgress = true; // block spin
      }
  ...
  const showGameMessage = (result, winAmount = '') => {
    ...
    switch (result) {
      ...
      case 'win':
        ...
        break;
      case 'loss':
        imageSrc = window.sadGhostImg;
        messageTitle = 'Not this time!';
        buttonText = 'Play again!';
        break;
      case 'selectSide':
        imageSrc = window.sadGhostImg;
        messageTitle = 'Choose a Pumpkin or a Ghost before starting the game.';
