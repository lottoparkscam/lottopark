 ...
 */
class Tests_E2e_Services_Raffle_Sync_Ticket extends Test_Feature
{
    ...
    public function test_it_synchronizes_data(): void
    {
        ...
        WhitelabelRaffleTicket::flush_cache();
        ...
        foreach ($synchronized_tickets as $ticket) {
            ...
            $this->assertNotEmpty($ticket->raffle_draw_id);
        ...
        # check raffle is enabled after full synchronization
        $this->raffle_dao::flush_cache();
        $raffle = $this->raffle_dao->get_by_slug_with_currency_and_rule(self::RAFFLE_SLUG);

        $this->assertTrue((bool)$raffle->is_sell_enabled);

        # summary to check the results
        print_r($balances);
    }
}
